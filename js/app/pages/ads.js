            ...
            <popup ref="copy" title="Copy banner">
                <div class="form inner-form">
                    <form v-if="parent.formData">
                        <div class="row">
                            <label>Code</label>
                            <textarea class="copyTextarea" v-model="parent.formData.copy"></textarea>
                        </div>
                        <div class="row">
                            <button class="btnS copyBtn" @click.prevent="copy(parent.formData.copy)">Copy code</button>
                        </div>
                    </form>
                </div>
            </popup>
                        ...
                        <tr v-for="item in data.items" :key="item.id">
                            ...
                            <td class="image">
                                <a href="#" @click.prevent="parent.formData = item; $refs.new.active=1;">
                                    <img :src="item.img ? (parent.url + item.img) : ''" style="max-width: 100px; max-height: 100px;" />
                                </a>
                            </td>
                            <td class="image">
                                <a href="#" @click.prevent="parent.formData = item; $refs.new.active=1;">{{item.campaign_title}}</a>
                            </td>
                            <td>
                                <a href="#" @click.prevent="parent.formData = item; $refs.new.active=1;">{{item.link}}</a>
                            </td>
                            <td class="actions">
