export const campaign = {
  ...
  methods: {
    ...
    get: function () {
      ...
      axios
        .post(
          this.parent.url + '/site/getBanners?auth=' + this.parent.user.auth,
          data
        )
    ...
    actionAd: function () {
      ...
      axios
        .post(
          this.parent.url + '/site/actionBanner?auth=' + this.parent.user.auth,
          data
        )
        .then(function (response) {
...
<div class="inside-content">
    ...
    <popup ref="chart" fullscreen="true" title="Chart">
        ...
        <div class="flex body">
            <div class="w30 ar filchart">
                <div class="itemchart ptble" v-if="all">
                    <toggle :modelValue="all" @update:modelValue="all = $event; checkAll($event)" />
                    ALL
                    <hr/>
                    <div class="itemchart ptble" v-if="data.items[iChart].sites" v-for="s in data.items[iChart].sites">
                        <toggle :modelValue="s.include" @update:modelValue="s.include = $event; parent.formData = data.items[iChart]; get()" />
                        {{s.site}}
                    </div>
                </div>
            </div>
    ...
    <div class="wrapper">
        ...
        <popup ref="details" fullscreen="true" title="Details">
            <div class="flex panel">
                <div class="w60"><input type="text" placeholder="Search..." v-model="q" @keyup="getDetails()" /> <input type="date" v-model="date" @change="getDetails()" /></div>
                <div class="w40 al">
                </div>
            </div>
            <br>
                    ...
                    <tr v-for="(item, i) in data.items">
                        ...
                        <td class="image">
                            <a href="#" @click.prevent="parent.formData = item; $refs.ad.active=1;">
                                <img class="imgBanner" :src="this.parent.url+'/'+item.img" />
                            </a>
                        </td>
